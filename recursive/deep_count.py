...
def deep_count(p):
    sum = 0 # sum of total of this list and all sublists
    for i in p:
        sum = sum + 1 # each element in the list will add 1 to sum
        if is_list(i): # if the element is a list
            sum = sum + deep_count(i) # add the number of element of the sublist to sum
    return sum
