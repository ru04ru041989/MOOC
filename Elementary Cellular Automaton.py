...
def get_rule(n):
    ''' for a given int n, return rule as a index'''
    patterns = {}
...
def cellular_automaton(string, pattern_number, generations):
    patterns = get_rule(pattern_number)
    
    for j in range(generations):
        new_string = ''
        for i in range(0,len(string)):
            if i +1 < len(string):
                str_in = string[i-1] + string[i] + string[i+1]
            else:
                str_in = string[i-1] + string[i] + string[0]
        
            new_string = new_string + patterns[str_in]
        string = new_string
